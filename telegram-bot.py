...
api_id = 23905794    #Your Api ID
api_hash = 'fa95fb37e11c5fcd7416c403d5ce791f'    #Your Api Hash
phone = '+905452108164'      # Your Phone Number With Country Code.
...
def add_users_to_group():
    ...
    filename = 'members-ninja-traders.csv'
    users = []
...
def list_users_in_group():
    ...
    with open("members-" + re.sub("-+","-",re.sub("[^a-zA-Z]","-",str.lower(target_group.title))) + ".csv","w",encoding='UTF-8') as f:
        ...
        writer.writerow(['username','user id', 'access hash','name','group', 'group id','status','isAdded'])
        for user in all_participants:
            ...
            writer.writerow([username,user.id,user.access_hash,name,target_group.title, target_group.id,user.status,'0'])      
    print('Members scraped successfully.')

def printCSV():
    input_file = 'members-aaaaa.csv'
    users = []
    with open(input_file, encoding='UTF-8') as f:
        rows = csv.reader(f,delimiter=",",lineterminator="\n")
        next(rows, None)
